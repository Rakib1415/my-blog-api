...
const createArticle = async({title, body, cover='', authorId, status='draft'}) => {
    const articleInstance = new Article(databaseConnection.db.articles);
    const article = await articleInstance.create({title, body, cover, authorId, status}, databaseConnection);
    return article;
